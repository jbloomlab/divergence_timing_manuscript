...
def run_phydms(alignment, prefs_fname, params_fname, tree_fname, beta, outprefix):
    ...
    # maximize likelihood
    print("max LL for beta {0}".format(beta))
    optimize_brlen = True
    printfunc = None
...
def real_data():
    ...
    for beta in betas:
        ...
        run_phydms(alignment, prefs_fname, params_fname, tree_fname, beta, outprefix)
...
def simulate_data():
    ...
    new_tree_name = "_temp_tree_long.newick"
    t = Tree(tree_fname)
    for node in t.traverse():
        node.dist = node.dist * 4
    t.write(format=1, outfile=new_tree_name)

    # first simulate the data
    simulate_prefix = "_temp_simulate"
    simulate_name = "{0}_simulatedalignment.fasta".format(simulate_prefix)
    if not os.path.isfile(simulate_name):
        model = create_model_ExpCM(params_fname, prefs_fname,1.5)
        phydmslib.simulate.simulateAlignment(model, new_tree_name,
                                             simulate_prefix, 0)

    # now reoptimize the bl
    for beta in [0.75]:
        outprefix = "_temp_{0}_simulate/".format(beta)
        run_phydms(simulate_name, prefs_fname, params_fname, tree_fname, beta, outprefix)
