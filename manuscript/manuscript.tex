...
Phylogenetics is commonly used to estimate the time since the divergence of two lineages. 
Under the assumption of a molecular clock~\citep{zuckerkandl1965}, the branch lengths on a phylogenetic tree can be transformed into estimates of time~\citep{yang2012molecular}. 
When a calibration point exists, these timing estimates of evolutionary events can be placed on an absolute timescale. 
...
For example, the integration of filoviruses into their host genomes indicate that the two main clades of filoviridae, ebolovirus and marburg virus, diverged from their common ancestor 7-12 million years ago---but the phylogenetic estimate of this divergence of ebolavirus and margburgvirus is only $\sim$10,000 years ago~\citep{carroll2013molecular, taylor2014evidence}. 
...
Most substitution models account for constraint by trying to statistically infer selection. 
Some models allow the rate of amino-acid substitution to vary among sites according to some statistical distribution~\citep{yang1994maximum,yang2000codon}\skhcomment{models from wertheim2001purifying?}. 
Other models account for this site-specific constraints via site-specific stationary state describing the frequency of every amino acid at every site in the protein~\citep{halpern1998evolutionary,yang2008mutation,rodrigue2010mutation,tamuri2012estimating,mccandlish2014modeling, le2008phylogenetic}

A protein's functional constraints can be measured experimentally rather than inferred statistically~\citep{dean2007mechanistic, harms2013evolutionary}. 
Recently, a new experimental approach called deep mutational scanning~\citep{fowler2014deep} has been developed which can measure the site-specific purifying selection quantitatively for all sites in a protein. 
...
Here we examined how ExpCM's and other forms of modeling purifying selection affects the estimates of deep divergence times of influenza virus hemagglutinin (HA). 
We found that models with site-specific measurements of purifying selection estimates longer deep branches. 
We found this extension of branch length is mostly independent from common method of account for purifying selection by rate variation. 
