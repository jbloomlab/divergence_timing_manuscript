...
\begin{abstract}
\noindent \skhcomment{$\leq$ 250 words, currently 164}   
...
\begin{itemize}
\item It is well known that different protein sites evolve under constraints
...
\end{itemize}
\item Differences in stationary state are manifested in rate differences (e.g., Spielman work), but the resulting differences in rate don't fully capture the site-specific stationary state
\item These are not mutually exclusive, you can imagine why there would be both
\end{itemize}
\end{itemize}
...
\begin{itemize}
\item Along a single branch, a substitution model transforms sequence divergence to branch lengths, which under a molecular clock is assumed proportional time
\item Every substitution model has a stationary state, which is the expected sequence divergence at very long time.  
\begin{itemize}
\item In the long tails the expected sequence divergence remains constant as time increases. 
\item The saturation point will affect long branches more than short branches.
\end{itemize}
\item Some model features (\ref{fig:model_feature}) affect \textit{how long} it takes to reach the stationary state, whereas others affect the stationary state itself: 
\begin{itemize}
\item Adding $\Gamma\omega$ rate variation affects the ``path" to the stationary state but not the stationary state itself. Importantly, the stationary state is identically for every site in the protein if there isn't site-specific amino-acid preferences. (green and blue)
\item Adding a non-uniform stationary state creates a unique stationary state for each site in the protein (red). Sites which can tolerate many mutations have saturation points which are quite similar to the GY94 saturation point. Constrained sites show a dramatic difference. At a constrained site, ExpCM will estimate a longer branch than the GY94 model because the overall sequence divergence at the site is expected to be small. 
\item This difference cannot be recapitulated by more complex modeling of the $\omega$ parameter. We can transform the GY94 model into a  site-specific model by inferring a unique $\omega$ value from the ExpCM for each site in the protein. This model (yellow) takes longer to reach the saturation point than the GY94 model but ultimately has the same divergence level at the saturation point. 
\end{itemize}
...
\begin{itemize}
\item Both the GY94 and the GY94+$\Gamma\omega$ underestimate long branches. The $\Gamma\omega$ rate variation scheme is not enough to prevent this underestimation at every long branches. 
\item Another uniform stationary state model, the averaged ExpCM, also underestimates long branches. This model derives some information from the experiments but lacks site-specificity. 
\item Non-uniform stationary state models accurately estimate long branches (both the ExpCM and ExpCM+$\Gamma\omega$
\end{itemize}
