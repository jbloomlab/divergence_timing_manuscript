...
def createDateDictionary(sequences):
    ...
    This function assumes that for a sequence collected in 2000 there is a
    string '2000_2000' somewhere in the header.
    ...
    for seq in sequences:  # go through each sequence
        ...
        if match:  # if a date is found, extract the exact date
            ...
            if match:
                ...
                if year not in range(1918, 2018):  # does the date makse sense?
                    pass
                elif year in datesDictionary.keys():  # add seq to dictionary
...
def translate_with_gaps(seq):
    """
    This function uses `phydmslib.constants` to translate a nucleotide seq.
    input: `str`
    output: `str`
    """
    prot_seq = []
