...
\skhcomment{I wonder if we should change this second sentence, because I'm not sure how many of the other cases for which there are actually other methods outside of viruses?Maybe it would be better to say: "For highly diverged sequences, such phylogenetic techniques often estimate surprisingly recent divergence times."Then the next sentence (as already written) talks about how for viruses there are other methods that show these are too recent.}
...
Mutation-selection models such as ExpCM can still incorporate an $\omega$ parameter~\citep{haddox2017mapping}, \skhcomment{I realized this citation is wrong. For the fact that ExpCM and mutation-selection models can still incorporate omega, we should cite the relevant Rodrigue / Lartillot paper and Bloom (2017).The Haddox et al (2017) should be cited for the fact that omega can still be drawn from a gamma. At that point, we should also cite relevant Rodrigue / Lartillot paper for their approach to do this.} which now represents the relative rate of nonsynonymous to synonymous substitution \emph{after} accounting for the constraints due to the site-specific stationary state.
...
This more realistic null model gives ExpCM's more power to detect diversifying selection~\citep{bloom2017identification}\skhcomment{rodrigue MBE}, for amino-acid change, which occurs at some sites in HA due to immune selection.
...
\skhcomment{add in which $\beta$ value I used.}
...
These models are generally implemented in a Bayesian framework, which avoids the overfitting problems associated with trying to make maximum-likelihood estimates of the thousands of parameters describing site-specific stationary states\skhcomment{overfitting problems associated with trying to make}. 
