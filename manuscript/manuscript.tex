...
The model most comparable to our ExpCM's is the codon mutation-selection model implemented in \texttt{phylobayes}, which we will refer to as pbMutSel. 
...
If the limits of the ExpCM's reflected in Figure X are simply because the deep mutational scanning do not accurately measure the "true" amino-acid preferences of HA across the tree, then we would expect the pbMutSel models (which derive their stationary states from the entire tree) to perform better.
...
First, the stationary state inferred by pbMutSel is similar to the average of the H1 and H3 preferences measured by deep mutational scanning. 
As \ref{fig:phylobayes}A shows, the site-specific amino-acid preferences estimated by pbMutSel are more correlated with the average amino-acid preferences from the H1 and H3 experiments than the individual deep mutational scanning experiments are to each other. 
...
Second, the results in \ref{fig:phylobayes}B show that the ExpCM+$\Gamma\omega$(H1+H3 avg prefs) and the pbMutSel model estimated similar branch lengths on the HA tree in \ref{fig:empirical_trees}. 
...
We used amino-acid preferences measured in deep mutational scanning of an H1 HA~\citep{doud2016accurate} and an H3 HA~\citep{lee2018deep} to define the $\pi_{r, A\left(x\right)}$ parameters~(\ref{eq:Frxy}) of the ExpCM's. 
...
Specifically, we used \texttt{phydms\_prepalignment} with the flag \texttt{--minidentity 0.3} to sequences with ambiguous nucleotides, premature stops, or frameshift mutations as well as redundant sequences.  
We also removed any codon sites which are not shared between the H1 and H3 deep mutational scanning reference sequences by aligning to sequence in \ref{suppfile:hybrid_sequence}. 
We subsampled the remaining sequences so there are $\ge 5$ sequences per subtype with $\le 1$ sequence per year per subtype. 
...
These two H1 sequences are early and late representatives of the longest continuous sampled \skhcomment{in evolution but not in time...} lineage of human influenza. 

\subsection*{Code and analysis.}
All code used is available at \url{https://github.com/jbloomlab/divergence_timing_manuscript}. 
For the main analysis, we used \texttt{phydms}~\citep{hilton2017phydms} version 2.2.dev2 (available at \url{github.com/jbloomlab/phydms}) to filter sequences and estimate branch lengths, \texttt{pyvolve}~\citep{spielman2015pyvolve} version 0.8.7 (available at \url{https://github.com/sjspielman/pyvolve}) to simulate the sequences and \texttt{phylobayes}~\citep{rodrigue2014site} version 1.8 (available at \url{https://github.com/bayesiancook/pbmpi}) to infer the pbMutSel model. 

We also used \texttt{ggplot2}~\citep{wickham2016ggplot2}, \texttt{ggtree}~\citep{yu2017ggtree}, and \texttt{ggseqlogo}~\citep{wagih2017ggseqlogo} for visualization.
...
These sequences from the H1, H2, and H3 subtypes were required to be in the final alignment. 
}
\end{suppfile}
...
HA sequences used in \ref{fig:empirical_trees}.  
}
\end{suppfile}
...
HA sequences for the ``low divergence from H1" tree in \ref{fig:compete}.  
}
\end{suppfile}
...
HA sequences for the ``low divergence from H3" tree in \ref{fig:compete}.  
}
\end{suppfile}
...
Homologous sites in WSN and Perth using sequential numbering starting at the second (first non-M) codon. 
}
\end{suppfile}
...
Deep mutational scanning preferences from \citet{doud2016accurate} for the homologous sites between the H1 reference sequence and the H3 reference sequence. 
}
\end{suppfile}
...
Deep mutational scanning preferences from \citet{lee2018deep} for the homologous sites between the H1 reference sequence and the H3 reference sequence. 
}
\end{suppfile}
...
The average deep mutational scanning preferences from \citet{doud2016accurate} ad \citet{lee2018deep}. 
}
\end{suppfile}
...
ExpCM model parameters fit using the amino-acid preferences from~\citet{doud2016accurate}(\ref{suppfile:H1_prefs}) and the ``low divergence from H1" tree (\ref{suppfile:alignment_lowH1})
}
\end{suppfile}
...
GY94 model parameters fit using the ``low divergence from H1" tree (\ref{suppfile:alignment_lowH1}). 
}
\end{suppfile}
...
GY94+$\Gamma\omega$ model parameters fit using the ``low divergence from H1" tree (\ref{suppfile:alignment_lowH1}). 
}
\end{suppfile}
