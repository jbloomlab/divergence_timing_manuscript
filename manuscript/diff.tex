...
%DIF DEL manuscript_original_submission.tex   Thu May 10 09:06:36 2018
%DIF ADD manuscript.tex                       Fri Aug 17 08:51:52 2018
...
\title{Modeling site-specific amino-acid preferences deepens phylogenetic estimates of viral \DIFaddbegin \DIFadd{sequence }\DIFaddend divergence} 
...
Here we will consider mutation-selection models where the site-specific selection is assumed to act solely at the protein level (different codons for the same \DIFdelbegin \DIFdel{protein }\DIFdelend \DIFaddbegin \DIFadd{amino acid }\DIFaddend are treated as selectively equivalent).
...
An alternative strategy \DIFdelbegin \DIFdel{of obtaining the }\DIFdelend \DIFaddbegin \DIFadd{to account for site-specific }\DIFaddend amino-acid \DIFdelbegin \DIFdel{preference parametersvia Bayesian inference~\mbox{%DIFAUXCMD
\citep{lartillot2004bayesian, rodrigue2014site}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{preferences is to formally consider them as random effects across sites, rather than parameters, and infer them using Bayesian methods~\mbox{%DIFAUXCMD
\citep{lartillot2004bayesian, rodrigue2014site}}%DIFAUXCMD
.
This strategy }\DIFaddend is discussed in the last section of the Results.
...
Given a single branch, a substitution model transforms sequence \DIFdelbegin \DIFdel{identity }\DIFdelend \DIFaddbegin \DIFadd{divergence }\DIFaddend into branch length.
Under a molecular-clock assumption, this branch length is proportional to time.
The transformation from sequence \DIFdelbegin \DIFdel{identity }\DIFdelend \DIFaddbegin \DIFadd{divergence }\DIFaddend to branch length is trivial when the sequence identity is high.
...
In this regime, the accuracy of the substitution model becomes critical for transforming sequence \DIFdelbegin \DIFdel{identity }\DIFdelend \DIFaddbegin \DIFadd{divergence }\DIFaddend into branch length.
...
The invariance of the asymptotic sequence identity under different schemes for modeling $\omega$ is a fundamental feature of the mathematics of \DIFdelbegin \DIFdel{reversible substitution models}\DIFdelend \DIFaddbegin \DIFadd{this type of reversible substitution model}\DIFaddend .
...
Although the nucleotide model used with \texttt{RAxML} to infer this tree topology is probably less accurate than codon models, the modular subtype structure of the HA phylogeny \DIFaddbegin \DIFadd{(the tree is clearly divided into widely separated clades) }\DIFaddend means that most of the phylogenetic uncertainty lies in the length of the long branches separating the subtypes rather than in the tree topology itself.
\DIFaddbegin \DIFadd{We note that other genes may have phylogenetic structures that are more prone to topological uncertainty. 
In such cases, the accuracy of the substitution model may be important for avoiding topological errors such as long-branch attraction~\mbox{%DIFAUXCMD
\citep{felsenstein1978cases,lartillot2007suppression}}%DIFAUXCMD
.
}\DIFaddend 
...
\DIFaddbegin \DIFadd{We expect that further improvements could be achieved by also incorporating other factors such as host-specific substitution rates~\mbox{%DIFAUXCMD
\citep{worobey2014synchronized} }%DIFAUXCMD
that are known to be important for modeling the evolution of viral genes such as HA.
}\DIFaddend 
...
But hope comes from experimental progress in measuring actual site-interdependent constraints on proteins~\citep{olson2014comprehensive,wu2016adaptation,steinberg2016shifting,li2016fitness}, combined with new methods for using these measurements to parameterize fitness functions~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{sailer2017detecting,otwinowski2018inferring,otwinowski2018protein}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{sailer2017detecting,otwinowski2018inferring}}%DIFAUXCMD
}\DIFaddend .
...
This work was supported by the NIAID and NIGMS of the NIH under grant numbers R01AI127893 and \DIFdelbegin \DIFdel{R01GM102198}\DIFdelend \DIFaddbegin \DIFadd{R35GM126911}\DIFaddend . 
