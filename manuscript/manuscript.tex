...
The models with $\Gamma\omega$ rate variation estimated longer branches than the same base model without this variation. 
For example, in \ref{fig:empirical_trees}, the GY94+$\Gamma\omega$ model estimated longer branches than the GY94 and the ExpCM+$\Gamma
\omega$(H1 prefs) estimated longer branches than the ExpCM(H1 prefs). 

The models with site-specific stationary states, the ExpCM's, estimated longer branches than the GY94 models. 
Both the ExpCM(H1 prefs) and ExpCM(H3) prefs estimated longer branches than the GY94 model. 
However, in contrast with the effect of adding $\Gamma\omega$ rate variation, the effect of site-specific stationary defined by deep mutational scanning preferences is not uniform across the entire tree. 
The effect is largest on the branches near the focal sequence of the deep mutational scan (\ref{fig:empirical_trees}). 
While the increase in length estimated by the ExpCM's has a strong local effect, the increase is in seen even in the presence of $\Gamma\omega$ rate variation. 
The ExpCM+$\Gamma\omega$ estimated longer branches than the GY94+$\Gamma\omega$. 
The site-specificity of the ExpCM's stationary state is clearly important for estimating the long branches on the HA tree. 
