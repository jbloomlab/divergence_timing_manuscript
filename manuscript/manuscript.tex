...
\begin{abstract}
\noindent  
\skhcomment{from EM: title change to ``Modeling site-specific amino-acid preferences deepens phylogenetic divergence time estimates of viral proteins"}
\skhcomment{from EM: I know you know better, but the abstract reads like you are conflating branch length estimates and calendar time. I suggest making the connection more explicit}
Molecular phylogenetics is often used to estimate the time since the divergence of modern gene sequences.
For highly diverged sequences, such phylogenetic techniques \skhcomment{EM removed ``can"} estimate surprisingly recent divergence times. 
...
However, the improvements from these site-specific but time-homogeneous and site-independent models are limited by the fact that a protein's amino-acid preferences gradually shift as it evolves\skhcomment{EM: I don't see how this is a counter-point for your argument yet-- your work is time-homogeneous too.}.
Overall, our work underscores the importance of modeling site-specific amino-acid preferences when estimating deep divergence times---but also shows the inherent limitations of approaches that fail to account for how these preferences shift over time\skhcomment{More about deep below, but here I just want to make sure that this deep and the "deepens" of the title are the same deep}. 
\end{abstract}
...
Molecular phylogenetics is commonly used to estimate the historical timing of evolutionary events \citep{yang2012molecular} \skhcomment{EM: Again, we need to hear about how BL estimates are converted to calendar time}.
\skhcomment{I think EM made this comment ``Can you make a more explicit argument as to why this might be a bad idea?" about the sentence above but it was unclear from the pdf. I narrowed it down to somewhere in the introduction before the DMS paragraph.} 
...
These experimentally informed codon models (ExpCM's) \skhcomment{ExpCM's vs. ExpCMs}generally exhibit much better phylogenetic fit than standard substitution models~\citep{doud2015site, hilton2017phydms, haddox2018mapping, lee2018deep}.

Here we examine how ExpCM's and other models of purifying selection estimate deep divergence times on a phylogenetic tree of influenza virus hemagglutinin (HA)\skhcomment{At this point I don't have a clear idea of what "deep" means in this context. At this point I just think deep = far back in time}. 
...
We also show that ExpCM's estimate similar branch lengths to a mutation-selection model that infers the amino-acid preferences from the natural sequence data rather than using values obtained in experiments.
...
(A) The dN/dS parameter, $\omega$, can be defined as one gene-wide average (orange triangle) or allowed to vary according to some statistical distribution \skhcomment{(blue circles)}. 
...
Under a molecular-clock assumption, this branch length is proportional to time\skhcomment{finally!!}.
...
\skhcomment{You've run into a huge personal pet peeve here. Topic sentences should make statements about content. Tell me what I need to know from this paragraph about how the models make different predictions, then the rest of the paragraph can back this up using further statements and parenthetical references to the figure.}
...
\skhcomment{This paragraph is much better, with a real topic sentence :+1} 
...
Importantly, models that account for purifying selection via either $\Gamma\omega$ rate variation or site-specific amino-acid preferences do not just exhibit better fit---they also estimate longer deep branches on the HA tree\skhcomment{The diameter analysis in this paragraph hasn't convinced me that the effect is especially pronounced for deep branches.}. 
...
However, while adding $\Gamma\omega$ rate variation increases the length of deep branches in a roughly uniform fashion across the tree, the branch lengthening from adding site-specific amino-acid preferences is not uniform across the tree (\ref{fig:empirical_trees}) \skhcomment{If you really want to make this point, I think you'll need to make plots or tests of some kind. My eyeball test is somewhat inconclusive.}.
\skhcomment{Second EM comment ``really, I'm just supposed to stare at this?"} 
...
At the most extreme value of $\beta = 0$, the preferences would be perfectly uniform and look similar to the GY94 preferences in \ref{fig:empirical_trees}\skhcomment{If you had some random preferences, would they really hit beta = 0?}.
...
If the limitations of ExpCM's described above arise simply because the deep mutational scanning experiments do not correctly measure the ``true" amino-acid preferences across the entirety of a highly diverged phylogenetic tree, then we would expect the pbMutSel models (which infer these preferences from the entire tree) to perform better.
...
\skhcomment{Overall, the effect seems kind of small compared to the size of problems you describe in the introduction. Do you really think that we can fix those time estimates with perfect modeling?

I really think you need to quantify the effects you see in figure 4 with plots rather than saying "look at the trees". If I missed something let me know.

Discussion: would it be possible to use a pbMutSel model, but fix its values to those corresponding to an ExpCM model at the point in the tree where the measurement was made? I don't know if you can make the two models talk to each other though.}
