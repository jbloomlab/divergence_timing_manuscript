...
%DIF ADD manuscript.tex                       Fri Aug 17 06:01:15 2018
...
The invariance of the asymptotic sequence identity under different schemes for modeling $\omega$ is a fundamental feature of the mathematics of \DIFaddbegin \DIFadd{this type of }\DIFaddend reversible substitution models.
...
Although the nucleotide model used with \texttt{RAxML} to infer this tree topology is probably less accurate than codon models, the modular subtype structure of the HA phylogeny means that most of the phylogenetic uncertainty lies in the length of the long branches separating the subtypes rather than in the tree topology itself\DIFaddbegin \DIFadd{.
This confidence in the }\texttt{\DIFadd{RAxML}}\DIFadd{(GTRCAT)-inferred tree is certainly not universal and in other cases more complex, biologically-realistic models may be necessary for accurate topology inference. 
For instance, others have shown that models with site-specific amino-acid preferences can avoid topological errors such as long-branch attraction~\mbox{%DIFAUXCMD
\citep{lartillot2007suppression}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
