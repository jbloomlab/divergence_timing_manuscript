...
\begin{abstract}
\textit{$\leq$ 250 words, currently 164}   

Molecular dating techniques are often used to estimate the origin date of modern, circulating viruses. 
These estimates are often substantially younger than estimates from methods which do not rely on viral phylogenies. 
...
Here, we show that substitution models informed by empirical measurements of mutational tolerance model constraint better than traditional models and extend branch lengths. 
...
\begin{itemize}
\item Substitution models are only useful for inferring branch lengths when they can accurately transform sequence divergence to time (branch lengths)
\item Every substitution model has a saturation point where this temporal signal is lost.  
\begin{itemize}
