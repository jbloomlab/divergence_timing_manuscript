...
However, other implementations allow the stationary stationary state of a site-specific model to be inferred directly from the natural sequence data. 
These models are generally implemented in a Bayesian framework, which avoids the overfitting problems associated with trying to make maximum-likelihood estimates of the thousands of parameters describing site-specific stationary sates. 
...
The \modelName{MutSel} model is mathematically identical to the ExpCM's except the amino-acid preferences are modeled using Dirichlet processes rather than derived from experiments. 
...
Comparing ExpCM's and \modelName{MutSel} models can therefore help to explain the limitations of the ExpCM's. 
Are the amino-acid preference sets defining the ExpCM's in \ref{fig:empirical_trees} poor descriptors of the site-specific natural selection across the whole tree, or is there simply not a single set that adequately describes this selection? 
If there is a single set, we would expect the \modelName{MutSel} model to outperform the ExpCM because its stationary state is not biased by the specific sequence used in an experiment. 
