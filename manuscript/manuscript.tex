...
\subsection*{Decay to stationary state}

I built the models using the results for the hybrid\_lowH1 tree. 
I used YNKGP M0, YNGKP M5, and the ExpCM with the hybrid doud preferences. 
The $\omega_r$ values were calculated without the fit $\omega$ from the ExpCM. 
When making the YNGKP M0 + $\omega_r$ the $\omega$ value is set to spielman\_wr * $\omega$ from the fit YNGKP M0. 
The preferences were rescaled with the same $\beta$ as the ExpCM. 
...
Used \texttt{pyvolve}. 
The tree was the large tree, specifically high hybrid with doud prefs 
total of 10 replicates 
Model was the hybrid lowH1 with hybrid doud prefs 
