...
Here we will use such substitution models in the form proposed by \citet{goldman1994codon}, which we denote GY94.
Using the classification scheme of \citet{yang2000codon}, when these models have a single gene-wide $\omega$ they are called GY94 M0.
This gene-wide $\omega$ is usually $<1$\jdbcomment{add citation, most of the site-specific $\omega$ papers will talk about how gene-wide is not sensitive, so we can cite some of those}, and crudely represents the fact that many amino-acid substitutions are under purifying selection.
...
It is challenging to obtain values for these parameters without overfitting the data\skhcomment{cite rodrigue paper}.
...
This $\omega$ parameter can be drawn from a distribution just like as for GY94-style models. 
We will denote ExpCM's that also include a distribution over $\omega$ as ExpCM+$\Gamma\omega$.
...
The transformation from sequence identity to branch length is trivial and insensitive to the choice of substitution model when the sequence identity is high.
For instance, in the case when where there has only been one substitution, then the sequence identity will simply be $\frac{L - 1}{L}$ for a gene of $L$ sites, and even a simple exponential model~\citep{zuckerkandl1965} will correctly infer the short branch length of $1/L$ substitutions per site.
...
\ref{fig:decay} shows how different substitution models predict sequence identity to decrease as a function of branch length for model parameters fit to a phylogeny of an H1 influenza hemagglutinin (HA).
The GY94 and GY94+$\Gamma\omega$ models each predict the same behavior for all sites, since they do not have any site-specific parameters.
Both of these models predict an asymptotic sequence divergence of \jdbcomment{?}, which is slightly higher than 0.05 since some of the 20 amino acids are favored by the models due to more redundant codons and biases towards certain nucleotides.
Intuitively, this asymptotic sequence identity seems low, since like many proteins HA has a highly conserved structure and function which imposes constraints that cause some sites to only sample a small subset of the 20 amino acids among all known HA homologs.

\jdbcomment{Then talk about we have a sense of these constraints from the DMS, and if we use them to put an $\omega_r$ it changes rate of approach of asymptote but not asymptote.
Then ExpCM with different asymptotes.
Then point about stationary state.}
