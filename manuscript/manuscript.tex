...
However, while adding $\Gamma\omega$ rate variation increases the length of deep branches in a roughly uniform fashion across the tree, the branch lengthening from adding site-specific amino-acid preferences is not uniform across the tree (\ref{fig:empirical_trees}) \jdbcomment{I think we should make a Figure 5 that looks similar to the current Figure 6B, that would show GY94 versus each ExpCM, GY94+gamma versus each ExpCM+gamma, and each model versus it's gamma. These could be panels (one or two rows) and panel B.} 
...
Therefore, \ref{fig:empirical_trees} shows that ExpCMs estimate longer branches, but that the effect is shaped by the set of amino-acid preferences used to inform the model.
