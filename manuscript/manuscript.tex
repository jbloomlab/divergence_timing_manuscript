...
\noindent\skhcomment{\textbf{reference summary}}

\noindent\skhcomment{\textit{how preferences shift over time:}
\begin{itemize} 
\item \citet{haddox2018mapping}: The amino-acid preferences from two diverged HIV env homologs are significantly shifted at several sites. 
\item \citet{ortlund2007crystal}: Small, permissive mutations are needed for a large functional/conformational change from ancestral to extant protein. 
\item \citet{gong2013stability}: Stabilizing mutations allow for otherwise destabilizing mutations to occur at T-cell epitopes in NP. 
\item \citet{harms2014historical}: Contingency is strong because only a few permissive genotypes exist. 
\item \citet{tufts2014epistasis}: Mutations in Hb from ancestral to extant sequences are highly dependent on the order. 
\item \citet{starr2018pervasive}: The evolutionary history of Hsp90 shows strong contingency/entrenchment and that the epistasis was primarily caused by specific interactions between sites. 
\item \citet{pollock2012amino}: Introduces the idea of evolutionary stokes shift - proteins tend to adjust to substitutions such that the propensity for that mutation is higher after the substitution than before it (making reversion less likely). 
\item \citet{doud2015site}: The amino-acid preferences from influenza NP homologs are shifted at a small number of sites.
\item \citet{shah2015contingency}: Most mutations which are selected for via purifying selection are epistatic, contingent on the previous substitutions, and become entrenched. Computational model. 
\item \citet{bazykin2015changing}: Review article. The set of amino-acids at a given position which confer the greatest fitness advantage change over time. 
\end{itemize}
}

\noindent\skhcomment{\textit{Phylogenetic models which account for shifting amino-acid preferences:}
\begin{itemize} 
\item \citet{rodrigue2005site}: Amino-acid model which accounts for the structural fitness of a sequence along with empirical amino-acid frequencies. 
\item \citet{choi2007quantifying}: Codon model which accounts accounts for solvent accessibility and pairwise interactions (tertiary structure) along with empirical amino-acid frequencies.
\item \citet{bordner2013new}: Modeling site interdependence using factor graphs. Similar mathematically structure to \citet{rodrigue2005site} but with a slightly different interpretation of the parameters. Amino acid
\item \citet{goldstein2017sequence}: Model based on statistical mechanics which describes the stability of a sequence as a function of the interaction of a given site an all other sites as well as a thermodynamic bath. 
\end{itemize}
}
...
\item \texttt{PhyloBayes-MPI}~\citep{rodrigue2014site} version 1.8 (available at \url{https://github.com/bayesiancook/pbmpi}) to \skhcomment{fit} the pbMutSel model. 
