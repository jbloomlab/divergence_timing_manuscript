...
\jdbcomment{First sentence not good.
The first part basically makes sense, but the second part doesn't follow.
Under a molecular clock you can estimate relative divergence times using phylogenetics.
Absolute timescales require a molecular clock \emph{plus} some absolute calibration points.
I would elaborate by breaking this into 2 or 3 sentences that independently discuss phylogenetics, molecular clock, and absolute timescale in a general fashion. 
Include some citations to general reviews or papers in this area.
Then have a paragraph break and go into next paragraph.
This will yield two shorter paragraphs that cleanly divide the general concept from the specifics about viruses.}
...
For example, integration of filoviruses into their host genomes indicate that the filoviridiae family diverged 7-12 million years ago\jdbcomment{rather than going ``... million years ago while the...'' instead break with a dash: ``... million years ago---but the phylogenetic estimate...''' This sentence structure more clearly draws the contrast. Also, the wording is a bit unclear about what is being dated: ``the filiviridae family diverged'''. Diverged from what? Then ``event'' in the next part. What ``event''? Choose more precise wording.} while the phylogenetic estimate of this event is only $\sim$10,000 years ago~\citep{carroll2013molecular, taylor2014evidence}.  
...
\jdbcomment{This ending is good, except that you have not introduced the nomenclature of branches or deep branches yet. Perhaps that could be remedied in the re-structured first paragraph above, which would take more time to talk about phylogenetics, what branch lengths mean, and absolute time in 2-3 sentences rather than all mashed together in one.} 
...
\jdbcomment{I have some more subtle wording suggestions the previous paragraph, but it is pretty good structure wise.
However, I don't think it transitions well to the next paragraph.
There is a bit of a conceptual gap.
Add something that bridges about how current approaches to better model this purifying selection are fundamentally statistical in that they try to \emph{infer} the purifying selection from the evolutionary process, such as by statistical distributions of rate variation or site-specific stationary states. 
Cite some papers for that.
You don't need to go into a lot of detail as that is covered in first Results section, but do mention that most approaches do this by trying to statistically infer selection.
Then transition into next paragraph about how these constrains can be measured experimentally.
However, for this paper, I think the first two general sentences about experiments aren't needed.
Just jump directly to deep mutational, talking about how this new experimental approach can measure the site-specific purifying selection quantitatively for all sites.}
...
Here we use ExpCM's to estimate branch lengths on an influenza hemagglutinin phylogenetic tree and find that these site-specific models estimate longer deep branches. \jdbcomment{Make this first sentence a bit broader, and say that we examine how using ExpCM's and other forms of modeling purifying selection affects estimates of deep divergence times of HA or something like that. Idea is correct, but the sentence seems a bit too narrow. Also, it is probably better to break into two sentences: one is what you investigate, and the second is what you find. Right now, both are in the same sentence.}
...
However, the results from these site-specific models are limited by an effect of purifying selection which is not accounted for in current models - \jdbcomment{Look up the difference between a hyphen and em-dash} how amino-acid preferences shift over time due to epistasis.
...
\jdbcomment{Make this paragraph a bit more general and less focused on ExpCM's.
The content is mostly good, but phrase it as we tested how rate variation and site-specific amino-acid preferences affect these things, and we used ExpCMs' to model preferences.
In general, the language should be a bit more general in Discussion and broader.}
...
\jdbcomment{I don't love the structure here. It is better to have one major point per paragraph. But the preceding paragraph has a topic sentence that is about the importance of preferences, but then the last part is about how they don't extrapolate well. That second point belongs in the next paragraph. The one above would be a good one to talk about why modeling prefs are important.}
...
\jdbcomment{Again, break preceding into two paragraphs for two points. The first point is that ExpCM's don't extrapolate well when you get too far away from the focal sequence. The next point (which should be a new paragraph) is that this is a \emph{general} feature of this type of model as shown by pbMutSeq.}

\jdbcomment{In the next paragraph, try to use the term shifting preferences rather than epistasis, as that is what we are talking about. The preferences do shift because of epistasis, so you can use that word. But we are not trying to use our models to accurately describe the effect of epistasis. We're trying to use them to model long-term evolution, and run into problems because preferences shift, which is due to epistasis.}
...
\jdbcomment{Make clear that there are two challenges associated with context-dependent models: evaluating them mathematically, and knowing the right function to model the shifting preferences. Right now these two challenges are not clearly separated in the logical flow.}
