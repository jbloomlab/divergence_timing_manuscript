...
Specifically, the tree's total height (distance from the two most divergent tips) is \jdbcomment{36\% needs to be updated for tree height} longer for the GY94+$\Gamma\omega$ model than for the GY94 model (\ref{supptab:tree_lengths}).
The total height is \jdbcomment{8\% needs to be updated for tree height} longer for ExpCM's informed by H1 or H3 amino-acid preferences than for GY94, and \jdbcomment{?} longer for the ExpCM informed by the average of the H1 and H3 preferences (\ref{supptab:tree_lengths}).
\jdbcomment{I am pretty sure that the standard quantity used in this context is not the average or total branch length, but rather the distance between the two most distant nodes on the tree, which is what I have called ``tree's total height'' here.
...
The near independence of these effects is quantified in \ref{supptab:tree_lengths}, which shows that \jdbcomment{96\%, needs to be fixed if using tree height} of the total tree height extension of ExpCM(H1+H3)+$\Gamma\omega$ versus can be explained by simply adding the extension from incorporating $\Gamma\omega$ (GY94+$\Gamma\omega$ versus GY94) to the extension from incorporating site-specific amino-acid preferences (ExpCM(H1+H3) versus GY94).
...
\jdbcomment{Change to be total tree height, which is distance between two most distant tip nodes.
...
Then add a new column that gives \% height relative to GY94 (the entry would just be 100\% for GY94).
So this gives 3 columns: model, total tree height, \% of GY94 tree height}
} 
     \begin{tabular}{cccccccccc}
        \hline
